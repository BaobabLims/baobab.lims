...
class IsMicroOrganismSampleKing(BrowserView):
    """ Drug vocabulary source for jquery combo dropdown box
    """

    def __init__(self, context, request):

        super(IsMicroOrganismSampleKing, self).__init__(context, request)
    ...
    def __call__(self):
        print('=====Print is micro organism')
        try:
            print(self.request.form)
            sample_kingdom_uid = self.request.form['sample_kingdom_uid']
            print(sample_kingdom_uid)

            brains = self.pc(UID=sample_kingdom_uid)
            sample_kingdom = brains[0].getObject()
            is_micro_organism = sample_kingdom.getField('IsMicroOrganism').get(sample_kingdom)
            print('--------sample kingdom')
            print(sample_kingdom)
            print(is_micro_organism)

            return json.dumps({'is_micro_organism': is_micro_organism})
