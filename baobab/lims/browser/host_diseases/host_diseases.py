...
class HostDiseasesView(BikaListingView):
    ...
    def __init__(self, context, request):
        ...
        self.contentFilter = {
            'portal_type': 'HostDisease',
        }
        self.context_actions = {_('Add'):
                                    {'url': 'createObject?type_name=HostDisease',
                                     'icon': '++resource++bika.lims.images/add.png'}}
        self.title = self.context.translate(_("Host Diseases"))
        ...
        if self.context.portal_type == 'HostDiseases':
            self.request.set('disable_border', 1)

        self.columns = {
            'Title': {
                'title': _('Host Disease'),
