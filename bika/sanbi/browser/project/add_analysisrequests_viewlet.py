...
class AddAnalysisRequestsViewlet(ViewletBase):
    index = ViewPageTemplateFile("templates/add_analysisrequests_viewlet.pt")
...
class AddAnalysisRequestsSubmitHandler(BrowserView):
    """
    """
    def __call__(self):
        ...
        if "viewlet_submitted" in self.request.form:
            ...
            if data:
                for x in range(data['seq_start'], data['seq_start'] + data['biospecimen_count']):
                    ...
                    analysisrequests.append(obj)

                msg = u'%s AnalysisRequests created.' % len(analysisrequests)
